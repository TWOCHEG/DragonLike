...
import pon.purr.utils.world.ExplosionUtility;
...
public class PlayerManager implements IManager {
    ...
    public void onSync(EventSync event) {
        ...
        if (mc.player.fallDistance > 0 && mc.player.getEquippedStack(EquipmentSlot.CHEST).getItem() == Items.ELYTRA) {
            ticksElytraFlying++;
    ...
    public void onTick(EventTick e) {
        if (Parent.fullNullCheck()) return;
        currentPlayerSpeed = (float) Math.hypot(mc.player.getX() - mc.player.lastRenderX, mc.player.getZ() - mc.player.lastRenderZ);
        ...
        for (Float value : speedResult) average += Math.clamp(value, 0f, 20f);
    ...
    public void postSync(EventPostSync event) {
        ...
        if (Purr.moduleManager != null && Purr.moduleManager.getModuleByClass(Rotations.class) instanceof Rotations r) {
            if (r.clientLook.getValue()) {
                mc.player.setYaw(yaw);
                mc.player.setPitch(pitch);
            }

            r.fixRotation = Float.NaN;
        }
    }
    ...
    private float getBodyYaw() {
        ...
        if (mc.player.handSwingProgress > 0.0f)
            offset = MinecraftClient.getInstance().player.lastYaw;
        float deltaBodyYaw = clamp(MathHelper.wrapDegrees((MinecraftClient.getInstance().player.lastYaw) - (bodyYaw + MathHelper.wrapDegrees(offset - bodyYaw) * 0.3f)), -45.0f, 75.0f);
        return (deltaBodyYaw > 50f ? deltaBodyYaw * 0.2f : 0) + MinecraftClient.getInstance().player.lastYaw - deltaBodyYaw;
    }

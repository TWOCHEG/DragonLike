package pon.main.modules.render;
...
public class FreeCam extends Parent {
    ...
    private final Setting<Float> hspeed = new Setting<>("v speed", 0.4f, 0.1f, 3f);
    ...
    public void onAttack(EventAttack e) {
        if (!getEnable() || fullNullCheck()) return;
    ...
    public float getFakeYaw() {
        return (float) Render2D.interpolate(prevFakeYaw, fakeYaw, Render3D.getTickDelta());
    }

    public float getFakePitch() {
        return (float) Render2D.interpolate(prevFakePitch, fakePitch, Render3D.getTickDelta());
    }

    public double getFakeX() {
        return Render2D.interpolate(prevFakeX, fakeX, Render3D.getTickDelta());
    }

    public double getFakeY() {
        return Render2D.interpolate(prevFakeY, fakeY, Render3D.getTickDelta());
    }

    public double getFakeZ() {
        return Render2D.interpolate(prevFakeZ, fakeZ, Render3D.getTickDelta());
    }
}
